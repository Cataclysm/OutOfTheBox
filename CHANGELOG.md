...
- Repository management: list/clone/delete of repositories under the configured root, with live size/git-status/active stats sampled on a background cadence; list and clone are also reachable as bearer-token REST endpoints (`GET /repositories`, `POST /repositories/clone`) so the sbx caller can query and provision repositories itself — deletion stays dashboard-only, since giving the sbx caller delete capability risks misuse
