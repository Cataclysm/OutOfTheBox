...
/// renders. <see cref="RepositoryPath"/> is the one exception - the repository-stats background sampler
/// (specs/repository-management) needs to know *which* repository to recompute on a terminal event
/// without a database round-trip just to find out.
/// </summary>
public sealed record RunEvent(Guid RunId, RunKind Kind, RunEventType Type, string RepositoryPath)
{
