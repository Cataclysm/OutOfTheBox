...
shared bearer token and either a `dotnet`/`git` argument list plus a repo-relative working
directory, or a request for a specific file inside a repo. The service checks the token, resolves
...
directory as its working directory. Commands against different repos run in parallel; a second
command against a repo that already has one in flight is rejected outright, not queued, so a caller
...
long silent wait. An artifact-transfer request instead streams a single file's raw bytes back,
confined to that specific repo's own directory (a second, narrower check than the working-directory
confinement above). Every run of every kind — `dotnet` command, `git` command, artifact transfer,
...
services that orchestrate them — run start/cancel, the per-repo lock registry) ← an outer ring split
...
- **History** — every past run, filterable by kind/outcome/repo and free-text searchable, with a
  per-run detail page showing full output and its resource-usage graph.
- **Repos** — every repository under the configured root, with live size/git-status/active state;
  clone a new one in or delete an existing one directly from here (this is the *only* way to manage
  repositories — there's no API for it, by design, so the sbx-side caller can never reach it).

## Running a Claude Code instance in the sbx sandbox

The sbx-side Claude Code instance is the caller this service exists for — it talks to the four REST
...
2. **Copy the skill into the sandbox's Claude Code environment.** The skill is authored and shipped
   in this repo, but installing it into a *different* machine's environment is a manual step this
   repo can't do for you — copy the `skills/dotnet-command-service/` directory (containing
...
output, cancel it, and pull back a produced artifact file — exactly as documented in the skill.
Repository management and the dashboard are both explicitly out of its reach (no REST surface for
either); those stay operator-only, from the dashboard above.
