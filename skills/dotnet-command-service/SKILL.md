...
`specs/service-authentication` - if the service's actual behavior ever seems to contradict this
doc, those spec files (not this one) are authoritative, and this doc is stale.

**Scope**: this skill covers the four endpoints below and nothing else. It does not cover the
operator-facing dashboard, host/process resource monitoring, or install/upgrade - none of that has
any bearer-token-authenticated API surface for you to reach. **Repository management (cloning or
deleting a repo) is explicitly out of scope too and cannot be done from here at all** - it has no
REST endpoint; it's a dashboard-only action the human operator performs, by design. If you need a
repo to exist before you can run commands against it, ask the operator - don't look for an
...
| `workingDirectory`  | yes, non-empty | A path relative to the service's configured root (e.g. just the repo name, `"my-repo"`, or a subdirectory within it, `"my-repo/src/Project"`). Must resolve *inside* that root - an escaping or absolute path is rejected. |
...
A `dotnet` run and a `git` run against the *same* repo share one lock, bidirectionally: starting a
`git pull` while a `dotnet test` is still running against that repo (or vice versa) gets rejected
as busy (see [Errors](#errors-and-rejections) below) - regardless of which kind was already there.
Two runs against *different* repos never contend with each other.
...
## Requesting an artifact: `POST /artifacts`

For pulling an actual output file a prior `dotnet`/`git` run produced (a built DLL, a test-results
file, etc.) - not for streamed console output, which you already got via SSE above.

```bash
curl -sk -D /tmp/headers.txt -X POST "$BASE_URL/artifacts" \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"repo": "my-repo", "path": "test-results/results.trx"}' \
  -o results.trx
```

Request body: `{"repo": "<same relative-path shape as workingDirectory above>", "path": "<file
path relative to that repo's own root>"}`. `X-Run-Id` is set on the response before the body starts
streaming, same as `/run`/`/run/git`.
...
`path` is confined to *that specific repo's own directory* - a path that escapes it (`../`,
...
to believe exist inside the repo you just ran a command against** (e.g. because you told `dotnet
test` to write its results there) - this endpoint does not offer directory listing, so there's no
way to discover what's available if you're guessing.
...
> inspecting this repo itself) - that pattern generalizes to any repo built here. If you need a
> file back via this endpoint, make sure whatever command produced it was told to write to an
> explicit path *inside* the repo (e.g. `dotnet test --results-directory <path-under-the-repo>`),
> not left to a default build-output location that might not be reachable from inside that repo's
> own directory tree at all.

## Cancelling a run: `POST /run/{runId}/cancel`

One endpoint for every cancellable kind - a `dotnet` run, a `git` run, or an in-flight artifact
...
- `202 Accepted` - cancellation was requested. The run's own stream (if you're still attached to
  ...
  `/run/git`) or simply end (for an artifact transfer, which has no SSE framing to write a
  terminal event into).
- `404 Not Found` - the run id is unknown to the server *right now*. This covers three distinct
  situations you can't tell apart from the response alone: the id never existed (e.g. a typo), the
  run already finished on its own before your cancel request arrived, or the service process
  itself restarted since the run started (its in-memory tracking is not persisted across a
  restart - a run that was genuinely still going before a restart is not cancellable afterward,
  though its history record does get reconciled to an interrupted state on the operator's side).

## Errors and rejections

| Situation | What you see |
|---|---|
| Missing/invalid bearer token | `401 Unauthorized`, empty body, on any of the four endpoints - happens before anything else is checked. |
...
| `/run`, `/run/git`: the target repo already has a run in flight | SSE `error`, `{"reason": "validation", "runId": "<the blocking run's id>"}` - the run already holding that repo's lock. Wait for it (or cancel it, if it's yours to cancel) before retrying. |
...
| `/artifacts`: missing `repo`/`path`, or either resolves outside its allowed root | `400 Bad Request`, empty body - checked, and responded to with a real HTTP status, *before* any streaming starts (unlike `/run`'s validation, this endpoint hasn't committed to a response yet at that point). |
| `/artifacts`: the resolved file doesn't exist (or is a directory - this endpoint never lists directory contents) | `404 Not Found`, empty body. Distinct from the 400 case above: 404 means "the path itself is fine, nothing's there (yet, or ever)"; 400 means "the path itself isn't allowed." |
| `/run/{runId}/cancel`: unknown/already-finished/pre-restart run id | `404 Not Found` (see [Cancelling a run](#cancelling-a-run-post-runrunidcancel) above for the three cases this covers). |

## Out of scope (do not look for these)

- **Repository management** (create/list/delete a repo) - dashboard-only, no REST endpoint, by
  design. Ask the operator.
