...
### The first cycle waits for `IHostApplicationLifetime.ApplicationStarted`, then runs immediately - added per direct instruction
Originally this sampler's first cycle simply waited for `PeriodicTimer`'s first tick like every subsequent one - meaning, at default settings, a freshly started service wouldn't fetch anything for a full 5 minutes, and (worse, the actual reason this was revisited) nothing prevented that first tick from landing while the host itself was still mid-startup (EF Core migration, certificate loading, other hosted services' own `StartAsync` still in flight) if a shorter interval were ever configured. Neither is desirable: a repository nobody has ever fetched shouldn't wait an arbitrary interval just because the service happened to just start, and a background network operation has no business running before the host is actually ready to serve.

Fixed by waiting on `IHostApplicationLifetime.ApplicationStarted` (a `CancellationToken` that fires once every hosted service's `StartAsync` has returned and, for a Kestrel-hosted app, the server is accepting connections - the standard, documented way to defer work until "the app is truly up") before the very first sweep, then running that first sweep immediately rather than also waiting for `PeriodicTimer`'s first tick. `RepositoryStatsSampler`'s own "compute once immediately" call was *not* changed to use the same gate - its work is purely local/cheap (no network, no dependency on the host being reachable from anywhere) and already existed before this consideration was raised; revisiting it was out of scope for this change.
