...
Lets an MCP caller list and clone repositories, mirroring exactly the REST-reachable subset of `repository-management` — nothing more — so the sbx sandbox caller gets the same two actions over MCP it already has over the bearer-token REST API, without gaining any of the dashboard-only capability that subset deliberately excludes.

## ADDED Requirements

### Requirement: list_repositories mirrors the REST repository inventory
The system SHALL accept a `list_repositories` tool call and return every repository under the configured root with the same identifying stats `repository-management` already requires for `GET /repositories`: name, total size, git status summary, and active/idle state.
...
The system SHALL accept a `clone_repository` tool call carrying a source URL, a name, and an optional initial branch, resolve the name under the configured root (rejecting a name that would escape the root or that already exists), and start `git clone` targeting that resolved directory the same way `repository-management` already requires for the REST clone endpoint — returning a run id immediately, in the same start-then-poll shape `mcp-command-execution` defines for `dotnet_run`/`git_run`, with progress and completion retrieved via that same capability's `read_run_output` tool.
...
Unlike the REST API's cancellation endpoint, which does not accept a repository-management run's id, `cancel_run` (per `mcp-command-execution`) SHALL accept the id of an in-flight `clone_repository` run and cancel it — a deliberate simplification for the MCP surface, since MCP has one cancellation tool covering every run kind it can start.
...
The system SHALL NOT expose repository deletion, pull/push/force-push/fetch/clean, branch switching, commit checkout, or the file tree browser's operations as MCP tools — these remain reachable only through the authenticated dashboard, per `repository-management`'s existing REST/dashboard boundary.
