...
- [x] 1.3 Scaffold `src/BuildAndTestService.Domain` (plain `net10.0` class library, no NuGet dependencies beyond the BCL) — **corrected**: originally `src/Domain/BuildAndTestService.Domain/` (double-nested); flattened to sit directly under `src/`
- [x] 1.4 Scaffold `src/BuildAndTestService.Application` (plain `net10.0` class library, references `Domain` only) — same flattening correction as 1.3
- [x] 1.5 Scaffold `src/BuildAndTestService.Infrastructure` (`net10.0-windows` class library, references `Application` + `Domain` only) — same flattening correction as 1.3
- [x] 1.6 Scaffold `src/BuildAndTestService.Presentation` (plain `net10.0` Razor Class Library — `Sdk="Microsoft.NET.Sdk.Razor"` — carrying `.razor` components, `wwwroot`, minimal API endpoint-mapping extension methods, and auth middleware; references `Application` + `Domain` only, **no reference to `Infrastructure`**) — same flattening correction as 1.3
- [x] 1.7 Scaffold `src/BuildAndTestService.Host` (`net10.0-windows` ASP.NET Core executable — the only project referencing all four others) — same flattening correction as 1.3
...
- [x] 2.1 Scaffold `tests/BuildAndTestService.UnitTests` (xUnit, `Microsoft.NET.Test.Sdk`), references `Domain`, `Application`, `Infrastructure` — no real process spawning, no real network; fakes/mocks for `Process`, `IRunEventBus`, `IResourceEventBus`, a real SQLite `:memory:` connection for repository-logic tests, and the internal `Domain`/`Application`/`Infrastructure` folder split all land as each layer gains real test content in later sections. **Corrections**: (a) from design.md — since it references `Infrastructure` (`net10.0-windows`), a plain `net10.0` project can't reference it — `UnitTests` (and `BehaviorTests`/`ArchitectureTests`, both of which reference `Host`/`Infrastructure`) target `net10.0-windows` too, not plain `net10.0`; (b) structural — originally `tests/UnitTests/BuildAndTestService.UnitTests/` (double-nested); flattened to sit directly under `tests/` (see §17)
- [x] 2.2 Scaffold `tests/BuildAndTestService.BehaviorTests` (Reqnroll.xUnit + `Microsoft.AspNetCore.Mvc.Testing`), references `Host`, for Gherkin `.feature`-file scenarios exercised against the real ASP.NET Core pipeline via `WebApplicationFactory<Program>`/`TestServer` — required exposing Host's implicit `Program` class via a trailing `public partial class Program;` so another assembly can reference it; same flattening correction as 2.1
- [x] 2.3 Scaffold `tests/BuildAndTestService.ArchitectureTests` (xUnit + `NetArchTest.Rules`), referencing all five projects (only for reflection over their assemblies, not for calling into them); same flattening correction as 2.1
...
- [x] 17.1 Flattened all five `src/` projects: `git mv src/<Layer>/BuildAndTestService.<Layer> src/BuildAndTestService.<Layer>`, removed the now-empty layer directories
- [x] 17.2 Flattened all three non-fixture `tests/` projects the same way (`tests/<TestType>/BuildAndTestService.<TestType>` → `tests/BuildAndTestService.<TestType>`); `tests/Fixtures/` untouched (categorically different - external-repo simulators, never solution-registered)
- [x] 17.3 Rewrote `BuildAndTestService.slnx`: removed the five `Domain`/`Application`/`Infrastructure`/`Presentation`/`Host` solution folders (projects now sit flat at the solution root); `Tests` remains the only solution folder, now wrapping flat (unfoldered) test projects; updated every `<Project Path>` to the new flat paths
