...
- [x] 7.3 Created `INSTALL.md`: today's dev-run instructions (`dotnet run --project src/Host/OutOfTheBox.Host`, required config keys), with a clearly-marked "Planned" section for §14's `install.ps1`/`upgrade.ps1` flow, to be rewritten with concrete instructions once that section lands
...
- [ ] 14.1 Configure `Host` for self-contained, single-file, win-x64 publish (`--self-contained true -p:PublishSingleFile=true -p:IncludeNativeLibrariesForSelfExtract=true`, trimming disabled); confirm publish targets only `src/Host/OutOfTheBox.Host`, not `Domain`/`Application`/`Infrastructure`/`Presentation` or the test projects individually (those are pulled in transitively as compiled dependencies and composed static web assets, not published as separate outputs)
- [ ] 14.2 Verify the published exe runs standalone on a clean Windows machine with no .NET runtime installed
- [ ] 14.3 Define the data directory layout (e.g. `%ProgramData%\OutOfTheBox\`) holding config and the SQLite file, separate from the install directory
...
- [x] 17.1 Flattened all five `src/` projects: `git mv src/<Layer>/OutOfTheBox.<Layer> src/OutOfTheBox.<Layer>`, removed the now-empty layer directories
- [x] 17.2 Flattened all three non-fixture `tests/` projects the same way (`tests/<TestType>/OutOfTheBox.<TestType>` → `tests/OutOfTheBox.<TestType>`); `tests/Fixtures/` untouched (categorically different - external-repo simulators, never solution-registered)
- [x] 17.3 Rewrote `OutOfTheBox.slnx`: removed the five `Domain`/`Application`/`Infrastructure`/`Presentation`/`Host` solution folders (projects now sit flat at the solution root); `Tests` remains the only solution folder, now wrapping flat (unfoldered) test projects; updated every `<Project Path>` to the new flat paths
- [x] 17.4 Fixed every `<ProjectReference>` path in all 7 affected `.csproj` files by hand (one directory level shallower each; recomputed rather than round-tripped through `dotnet remove/add reference` to avoid touching unrelated formatting)
- [x] 17.5 Added `Directory.Packages.props` (Central Package Management) at the repo root: all 9 package IDs used anywhere in the repo declared once as `<PackageVersion>`; stripped the `Version` attribute from every `<PackageReference>` in all 9 affected `.csproj` files, including the three `tests/Fixtures/` projects (CPM applies by directory walk-up regardless of `.slnx` membership)
- [x] 17.6 Added `.editorconfig` at the repo root: charset/EOL/indentation basics plus a handful of C# style conventions already in use throughout the codebase (file-scoped namespaces, `var` preference, `_camelCase` private fields) — all at `:suggestion` severity, since `CS1591` already covers the one rule that needed to be build-breaking
- [x] 17.7 Updated `openspec/changes/sbx-dotnet-command-service/{proposal,design}.md` to describe the flat layout, and to add the CPM/`.editorconfig` decisions; updated `README.md`/`BUILD.md`/`CLAUDE.md` path references; added a `CHANGELOG.md` entry
- [x] 17.8 Confirmed clean `dotnet build`/`dotnet test` after every step (flattening, reference fixes, CPM, editorconfig) — 58/58 tests passing throughout, plus a standalone build check that a `tests/Fixtures/` project still restores correctly under CPM despite not being solution-registered
