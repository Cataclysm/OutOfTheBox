...
A Windows-hosted service that lets a Claude Code instance running in a remote sbx sandbox run `dotnet build`/`test`/etc. against repos on this host over MCP (Streamable HTTP transport), since the sandbox has no local .NET toolchain. A Blazor Server web dashboard is the only other interface (operator-only, cookie-authenticated) - no client-side skill doc, since MCP tools are self-describing. Windows-only throughout (Windows Service hosting, `PerformanceCounter`, WMI). Licensed under [AGPL-3.0](LICENSE) - a modified version, including one only ever run as a hosted service, must make its source available under the same license.

The authoritative source for *why* and *what's left* is the `openspec/` directory: `openspec/specs/*/spec.md` holds the current, canonical behavior contracts for every shipped capability; `openspec/changes/<name>/` holds each individual change's own `proposal.md` (what/why), `specs/*/spec.md` (that change's behavior delta), `design.md` (architecture rationale and every non-obvious technical decision), and `tasks.md` (its live, checkbox-tracked implementation plan) - archived changes move to `openspec/changes/archive/<date>-<name>/` once merged into the canonical specs, so check there for one that isn't listed directly under `openspec/changes/`. Read a change's `design.md` before making a design-level change in its area — it documents *why* each decision was made, including alternatives that were rejected and why.
...
- **Copyright header**: every `.cs` file under `src/`, `tests/OutOfTheBox.{UnitTests,BehaviorTests,ArchitectureTests}`, and `installer/OutOfTheBox.Msi.CustomActions{,.Tests}` starts with:
  ```
  // Copyright (c) 2026 Dennis Freise <dennis.freise@final-frontier.org>
  // Licensed under the GNU Affero General Public License v3.0 or later - see LICENSE in the project
  // root, or <https://www.gnu.org/licenses/agpl-3.0.html>, for the full text.
  ```
  followed by a blank line. Do **not** add this to `tests/Fixtures/` (those simulate arbitrary external repos, not this project's own source) or generated code (EF Core migrations, Reqnroll `*.feature.cs`).

  The same notice, same three lines, goes at the top of every other hand-authored source file under `src/OutOfTheBox.Presentation` too (`.razor`, `.css`, `.js`) - just with each language's own comment syntax: `@* ... *@` for `.razor` (Razor comments emit nothing, so this stays invisible in rendered HTML), `/* ... */` for `.css`, `//` for `.js`. Never add it to a vendored third-party file (`wwwroot/{css,js}/vendor/*`, the Lucide SVGs under `wwwroot/icons/`) - those carry their own upstream license instead (see `wwwroot/icons/ATTRIBUTION.md`).
...
- **`.editorconfig`** at the repo root covers formatting and modern-C#-construct style conventions (pattern matching, target-typed `new`, collection expressions, readonly/accessibility modifiers, unused usings/parameters, ...), enforced at `warning` severity and surfaced during `dotnet build` via `Directory.Build.props`'s `EnforceCodeStyleInBuild` — `CS1591` (doc comments) remains the only rule promoted all the way to a build-breaking error. A couple of rules are deliberately left off/at default despite being available (documented inline in `.editorconfig`): primary constructors (poor fit for this codebase's multi-dependency DI-injected classes) and the statement-expression/ternary-over-return rules (too noisy or push toward less-readable throw-in-ternary code). EF Core's `Migrations/` folder is exempted (`generated_code = true`) since it's scaffolded, not hand-authored.
